...
def day_time(tm):
    ...
    if tm.count('/') == 3:
        ...
        for i in tm:
            ...
            cnt += 1
        y, m, d, h = tm.split('/')
        tm = '/'.join([y, m.lstrip('0'), d.lstrip('0'), h.lstrip('0')])
        return tm
    else:
        exit("日期格式有误!")
...
def sign():
    ...
    print("也可以输入其他平台")
    print("难度可以是任意正实数")
    print("时间为d自动获取, 或者手动填入")
    print("手动填入以/或-为分隔符, 如年/月/日/时")
...
def single(df):
    ...
    level = float(input())
    print("时间为d自动获取, 或者手动填入")
    print("手动填入以空格或/或-为分隔符, 如年 月 日 时")
    print("完成题目的时间为: ", end = '')
    day = input()
    df_new = pd.concat([df, pd.DataFrame([[plat, name, title, level, day_time(day)]], columns = df_col)], ignore_index = True)
    return df_new
...
def multi(df, n):
    ...
    while n > 0:
        n -= 1
        sentence = input()
        plat, name, title, level, day = sentence.split(' ')
        plat = parse_plat(plat)
        level = float(level)
        tmp = pd.concat([tmp, pd.DataFrame([[plat, name, title, level, day_time(day)]], columns = df_col)], ignore_index = True)
...
def file_input(df):
    ...
    tmp = pd.DataFrame(columns = df_col)
    for line in fin:
        plat, name, title, level, day = line.split(' ')
        plat = parse_plat(plat)
        level = float(level)
        tmp = pd.concat([tmp, pd.DataFrame([[plat, name, title, level, day_time(day)]], columns = df_col)], ignore_index = True)
    df_new = pd.concat([df, tmp], ignore_index = True)
